 ...
 *      $Revision: 1.176 $      $Date: 2014/08/20 16:46:16 $
 *
 ***************************************************************************
 * DESCRIPTION:
  ...
  if (can_write_angles() && m->test_dataset_flag(BaseMolecule::ANGLES)) {
    ...
    for (i=0; i<numangles; i++) {
      ...
      if ((idx0 >= 0) && (idx1 >= 0) && (idx2 >= 0)) {
        angles.append(idx0+1); // 1-based atom index map
        angles.append(idx1+1); // 1-based atom index map
        angles.append(idx2+1); // 1-based atom index map
#if vmdplugin_ABIVERSION >= 16
    ...
    for (i=0; i<numdihedrals; i++) {
      ...
      if ((idx0 >= 0) && (idx1 >= 0) && (idx2 >= 0) && (idx3 >= 0)) {
        dihedrals.append(idx0+1); // 1-based atom index map
        dihedrals.append(idx1+1); // 1-based atom index map
        dihedrals.append(idx2+1); // 1-based atom index map
        dihedrals.append(idx3+1); // 1-based atom index map
#if vmdplugin_ABIVERSION >= 16
        if (m->test_dataset_flag(BaseMolecule::ANGLETYPES)) {
    ...
    for (i=0; i<numimpropers; i++) {
      ...
      if ((idx0 >= 0) && (idx1 >= 0) && (idx2 >= 0) && (idx3 >= 0)) {
        impropers.append(idx0+1); // 1-based atom index map
        impropers.append(idx1+1); // 1-based atom index map
        impropers.append(idx2+1); // 1-based atom index map
        impropers.append(idx3+1); // 1-based atom index map
#if vmdplugin_ABIVERSION >= 16
        if (m->test_dataset_flag(BaseMolecule::ANGLETYPES)) {
