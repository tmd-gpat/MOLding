...
*      $Revision: 1.87 $         $Date: 2014/12/21 19:36:00 $
*
***************************************************************************
* DESCRIPTION:
...
// output image framebuffer, accumulation buffer, AA random number
// state buffer, and associated ray launch parameters
#ifndef VMDOPTIX_VCA
//#define USE_AA_RNG_BUFFER 1
//#define USE_AO_RNG_BUFFER 1
#if defined(USE_AA_RNG_BUFFER)
rtBuffer<unsigned int, 2> aa_rng_buffer;
#endif
#if defined(USE_AO_RNG_BUFFER)
rtBuffer<unsigned int, 2> ao_rng_buffer;
#endif
#endif
...
#else
rtDeclareVariable(unsigned int, progressiveSubframeIndex, , );
#endif
...
//
// Clear the AA and AO RNG buffers to identical initial seed values
//
RT_PROGRAM void clear_rng_buffers() {
#ifndef VMDOPTIX_VCA
#ifdef USE_AA_RNG_BUFFER
  aa_rng_buffer[launch_index] = 3133731337;
#endif
#ifdef USE_AO_RNG_BUFFER
  ao_rng_buffer[launch_index] = 3133731337;
#endif
#endif
}
...
//
// Clear the framebuffers to zeros
//
RT_PROGRAM void clear_framebuffer() {
#ifndef VMDOPTIX_VCA
  framebuffer[launch_index] = make_uchar4(0u, 0u, 0u, 0u);
#endif
}
...
#if defined(USE_AA_RNG_BUFFER)
  // read previous RNG state from global memory...
  unsigned int randseed = aa_rng_buffer[launch_index];
#else
  unsigned int randseed = tea<4>(launch_dim.x*(viewport_idx_y)+launch_index.x, progressiveSubframeIndex); 
#endif
...
#if defined(USE_AA_RNG_BUFFER)
  // write new RNG state back to global memory...
  aa_rng_buffer[launch_index] = randseed;
#endif
...
#ifdef USE_AO_RNG_BUFFER
  // read previous AO RNG state from global memory...
  unsigned int randseed = ao_rng_buffer[launch_index];
#else
  // unsigned int randseed = 3133731337U;
  unsigned int randseed = tea<2>(progressiveSubframeIndex, progressiveSubframeIndex); 
#endif
...
#ifdef USE_AO_RNG_BUFFER
  // write new AO RNG state back to global memory...
  ao_rng_buffer[launch_index] = randseed;
#endif
...
// all-on backwards compatibility with the original code 
// until the new scene and material flag system is completed
static __device__ void full_shade(float3 p_obj_color, float3 N, 
                                  float p_Ka, float p_Kd, float p_Ks,
                                  float p_phong_exp, float p_reflectivity,
                                  float p_opacity,
                                  float p_outline, float p_outlinewidth,
                                  int p_transmode) {
#if 1
  // don't need reflection normally
  shader_template<1, 1, 1, 1, 0, 1>(p_obj_color, N, p_Ka, p_Kd, p_Ks,
                                    p_phong_exp, p_reflectivity, p_opacity, 
                                    p_outline, p_outlinewidth, p_transmode);
#else
  // all options enabled, intended for shader debugging
  shader_template<1, 1, 1, 1, 1, 1>(p_obj_color, N, p_Ka, p_Kd, p_Ks,
                                    p_phong_exp, p_reflectivity, p_opacity, 
                                    p_outline, p_outlinewidth, p_transmode);
#endif
}
...
RT_PROGRAM void closest_hit_radiance() {
  full_shade(obj_color, calc_ffworld_normal(), Ka, Kd, Ks, phong_exp, Krefl,
